...
public class ScrabbleScore {
    public static int scrabbleScore(String word) {
        int result = 0;
        if (!word.equals("")) {
            String s = word.replaceAll("\\W","").toUpperCase()
                            .replaceAll("[AEIOULNRST]", "1 ")
                            .replaceAll("[DG]", "2 ").replaceAll("[BCMP]", "3 ")
                            .replaceAll("[FHVWY]","4 ").replaceAll("K", "5 ")
                            .replaceAll("[JX]", "8 ").replaceAll("[QZ]", "10 ")
                            .trim();
